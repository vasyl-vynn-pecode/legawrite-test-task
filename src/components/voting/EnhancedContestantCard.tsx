...
export function EnhancedContestantCard({
  contestant,
  hasVoted,
  canVote,
  remainingVotes,
  onVoteSuccess,
  onVoteError,
}: EnhancedContestantCardProps) {
  // Use custom hook for isolated contestant voting logic
  const { isVoting, voteStatus, error, submitVote, resetError } = useContestantVoting({
  ...
  const handleVote = () => {
    if (canVote && !hasVoted && !isVoting && contestant.isActive) {
      submitVote();
    }
  };

  // Show error state if voting failed
  React.useEffect(() => {
    if (error) {
    ...
    >
      ...
      >
        ...
        <CardFooter className="p-4 pt-0">
          <Button
            variant={voteStatus === 'error' ? 'destructive' : 'vote'}
            size="lg"
            className="w-full"
            onClick={handleVote}
